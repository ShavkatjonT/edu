...
class PendingGroupsController {
    async PendingGroupsAdd(req, res, next) {
        try {
            ...
            const text = `${name} guruh  kutush zalida yaratildi`;
    ...
    async PendingGroupsDelete(req, res, next) {
        try {
            ...
            const text = `${pendingGroup.name} guruh o'chirildi. Kutush zalida.`;
            await logSystemController.logsAdd({ reqdata: req, text });
    ...
    async PendingGroupsPut(req, res, next) {
        try {
            ...
            const text = `${pendingGroup.name} guruh nomi o'zgartirildi`;
            await logSystemController.logsAdd({ reqdata: req, text });
    ...
    async GroupsCreateGroupTable(req, res, next) {
        try {
            ...
            if (!pendingGroupId || !validateFun.isValidUUID(pendingGroupId)) {
                return next(
            ...
            const text = `${pendingGroup.name} nomli kutush zalidagi guruh ishga tushurildi`;
            await logSystemController.logsAdd({ reqdata: req, text });
