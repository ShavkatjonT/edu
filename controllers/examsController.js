...
class ExamsController {
  async examAdd(req, res, next) {
    try {
      const { name, date, summa_self, summa_other } = req.body;
      if (!name) {
  ...
  async examPut(req, res, next) {
    try {
      const { id, name, date, summa_self, summa_other } = req.body;
      if (!id) {
        return next(ApiError.badRequest("No data found"));
      }
      const exam = await Exams.findOne({ where: { id, status: "active" } });
...
Vim`,
  ...
  async examStudentPointAdd(req, res, next) {
    try {
      ...
      const lastActiveData = await ExamStudentPoint.findOne({
        ...
        order: [['created_at', 'DESC']], // Order the results by the 'createdAt' column in descending order
  ...
  async examRoomsSendMessege(req, res, next) {
    try {
      const id = 'e88fc98e-7703-479f-aa00-9e650e19cbc7'
      const exam = await Exams.findOne({
        where: { id, status: "active" },
      });
      if (!exam) {
      ...
      for (const datas of newData[0]) {
        const sendData = [
          {
            text: `${datas.student.firstname + ' ' + datas.student.lastname + ' ' + formatDate2(exam.date)}.kuni soat 6-00 da ${datas.room.name.split(' ')[0]}da test topshiradi.
Vim`,
      ...
      for (const datas of newData[1]) {
        const sendData = [
          {
            text: `${datas.student.firstname + ' ' + datas.student.lastname + ' ' + formatDate2(exam.date)}.kuni soat 9-00 da ${datas.room.name.split(' ')[0]}da test topshiradi.
Vim`,
            phone: datas.student.phone
          }
        ]
      ...
      if (newData[2]) {
        for (const datas of newData[2]) {
          const sendData = [
            {
              text: `${datas.student.firstname + ' ' + datas.student.lastname + ' ' + formatDate2(exam.date)}.kuni soat 11-00 da ${datas.room.name.split(' ')[0]}da test topshiradi.
Vim`,
              phone: datas.student.phone
            }
          ]
