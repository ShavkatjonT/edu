...
class MessagesController {
  ...
  async messageAddList(req, res, next) {
    try {
      ...
      if (!studentIdList) {
        ...
        if (studentIdList.find((el) => !validateFun.isValidUUID(el))) {
          return next(ApiError.badRequest("student idda hatolik bor"));

        }
      }
      if (!group_id || !validateFun(group_id)) {
        return next(ApiError.badRequest("group idsi yo'q "));
      } else {
        const groupOne = await Groups.findOne({
          where: { id: group_id, status: "active" },
  ...
  async messageGet(req, res, next) {
    try {
      ...
      const pendingGroups = await DndColumns.findAll({
        where: {
          status: 'active'
        }
      });
