const { Debtors, GroupStudents, Teachers } = require("../models/models");
...
class cronController {
  async eachStudents(student) {
    return new Promise(async (resolve) => {
      ...
      if (student.wallet > 0) {
        ...
        debt -= student.wallet;
        let gr_student = await GroupStudents.findOne({
          where: student.group_student_id,
        });
        ...
        const teacher = await Teachers.findOne({
          where: { id: student.teacher_id, status: 'active', job_type: 'teacher' },
        });
        if (teacher) {
          ...
          const sendData = [
            {
              phone: student.fatherPhone
                ? student.fatherPhone
                : student.motherPhone,
              text: `"Vim" o'quv markazi: Eslatib o'tmoqchimiz farzandingiz ${student.name} ${student.amount} so'm qarzdorligi mavjud. Iltimos qarzdorlik summasini to'lang.`,
            },
          ];
  ...
  async secondSms() {
    try {
      ...
      students &&
        students[0].forEach((el) => {
          const gr_student_one = gr_student.filter(
          ...
          const sendData = [
            {
              ...
              text: `Farzandingizning ${monthFun(
                (monthOne <= 9 ? "0" : '') + "" + monthOne
              )} oyi uchun ${amount} so'm miqdorda to'lovini oyning 5-sanasigacha amalga oshirishni unutmang! Vim`,
            },
          ];
